...
Figure 9. A) Alongshore dune growth and erosion for Bogue Banks for 1997-2016 (green), 1997-2010 (red),
and 2010-2016 (blue). B) Density distributions of vertical dune growth for Bogue Banks for 1997-2016, 1997-2010,
and 2010-2016. Positive values indicate vertical dune growth, negative values indicate vertical dune erosion.
C) Mean Dhigh elevation change for the entire island, Fort Macon, non-fenced areas, and fenced areas color
by time period (with 95% confidence intervals).
...
def change_df(d1997, d2010, d2016):
    ...
    for interval in intervals:
        ...
        # Some data may show up as negative if progradation occured due to how the morphometrics
        # were calculated. Multiply the change rate by -1 in order to associate progradation with positive change
        new_df[interval] *= -1
...
def shoreline_change_df(shores, rate=False):
    """
    Calculate the average rate of shoreline change for each interval and its associated
    ...
    new_df = shores[['1997', '2010', '2016', '1997-2010', '1997-2016', '2010-2016']]
    ...
    for interval in intervals:
        ...
        # Calculate the change rate between the years being analyzed. Multiply
        # change by negative 1 to have progradation be a positive value
    ...
    save_name = os.path.join('..', 'Data', 'Shorelines.csv')
...
def make_stats(data, avg, fences, param):
    ...
    # Create a .txt file
    partial_fname = f'(Figure 9) {param} Change Statistics.txt'
...
def main():
    ...
    # Load the data
    data_1997 = load_data(year=1997, variable='x_mhw')
    data_2010 = load_data(year=2010, variable='x_mhw')
    data_2016 = load_data(year=2016, variable='x_mhw')

    # Make a dataframe of Dhighs
    shorelines = change_df(d1997=data_1997, d2010=data_2010, d2016=data_2016)
    shorelines_errors = shoreline_change_df(shores=shorelines, rate=False)
    ...
    make_overall_change_plot_w_kde(data=shorelines,
                                   fences=fences,
                                   param='Shoreline',
                                   y_min=y_min,
                                   y_max=y_min * -1,
    ...
    make_stats(shorelines, shorelines_errors, fences, 'Shoreline')
