...
Figure 8. A) Alongshore beach width change for Bogue Banks for 1997-2016 (green), 1997-2010 (red),
and 2010-2016 (blue). B) Density distributions of beach width change for Bogue Banks for 1997-2016,
1997-2010, and 2010-2016. Positive values indicate progradation, negative values indicate erosion.
C) Mean beach width change for the entire island, Fort Macon, non-fenced areas, and fenced areas
colored by time period (with 95% confidence intervals).
...
def beach_width_change_df(widths, rate=False):
    """
    Calculate the average rate of change for the beach width for each interval and its associated
    ...
    new_df = widths[['1997', '2010', '2016', '1997-2010', '1997-2016', '2010-2016']]

    # At this point, there is a column for the x-position of the shoreline for every year.
    ...
    for interval in intervals:
        ...
        # Calculate uncertainty in the MHW position
        up1, up2 = mhw_uncertainty(begin_time, end_time)

        # Calculate uncertainty in the dune toe position
        up3, up4 = dlow_uncertainty(begin_df, end_df, begin_time, end_time)

        # Calculate the rate uncertainty and regional uncertainty
        # based on the positional uncertainty
        position_uncertainty = np.sqrt((up1 ** 2) + (up2 ** 2) + (up3 ** 2) + (up4 ** 2))
    ...
    save_name = os.path.join('..', 'Data', 'Beach Widths.csv')
...
def dlow_uncertainty(d0, df, t0, tf):
    """
    Calculate uncertainty in the position of the dune
    toe, based on methods in Hapke et al (2011)
    ...
    # Load the y_mhw values and beach slopes for the current years to calculate uncertainty
    data_begin = load_data(year=int(t0), variable=['x_toe', 'Foreshore Slope'])
    data_end = load_data(year=int(tf), variable=['x_toe', 'Foreshore Slope'])

    # Find the uncertainty in the position of the dune toe
    toe_error_yr1 = lidar_vertical_error(str(t0)) / data_begin['Foreshore Slope']
    toe_error_yr2 = lidar_vertical_error(str(tf)) / data_begin['Foreshore Slope']

    # Find uncertainty based on deviation from the average
    toe_mean_yr1 = np.nanmean(data_begin['x_toe'].dropna())
    toe_mean_yr2 = np.nanmean(data_end['x_toe'].dropna())
    toe_error_2_yr1 = abs(data_begin['x_toe'] - toe_mean_yr1)
    toe_error_2_yr2 = abs(data_end['x_toe'] - toe_mean_yr2)

    # Find uncertainty based on the confidence interval
    _, _, ci0 = confidence_intervals(d0)
    _, _, cif = confidence_intervals(df)

    # Find the positional uncertainty for each year
    up1 = np.sqrt((toe_error_yr1 ** 2) + (toe_error_2_yr1 ** 2) + (ci0 ** 2))
    up2 = np.sqrt((toe_error_yr2 ** 2) + (toe_error_2_yr2 ** 2) + (cif ** 2))
...
def mhw_uncertainty(t0, tf):
    """
    Calculate uncertainty in the position of MHW on the profile
    based on methods in Hapke et al. (2011)

    - begin: dataframe of shoreline positions for the beginning time
    - end: dataframe of shoreline positions for the end time
    - t0: beginning time
    - tf: end time
    ...
    # Load the y_mhw values and beach slopes for the current years to calculate uncertainty
    data_begin = load_data(year=int(t0), variable=['y_mhw', 'Beach Slope'])
    data_end = load_data(year=int(tf), variable=['y_mhw', 'Beach Slope'])

    # Use the confidence intervals as the first component of the error
    ci_df = pd.read_csv(os.path.join('..', 'Data', 'Shoreline Confidence Intervals.csv'))

    # Find the uncertainty in Y as the difference between the measured y-value
    # and the accepted value of MHW (0.34m). Do this for both years
    mhw_real = 0.34
    y_error_yr1 = abs(data_begin['y_mhw'] - mhw_real)
    y_error_yr2 = abs(data_end['y_mhw'] - mhw_real)

    # Find the uncertainty in X as the vertical error in the LiDAR
    # data divided by the beach slope
    x_error_yr1 = lidar_vertical_error(str(t0)) / data_begin['Beach Slope']
    x_error_yr2 = lidar_vertical_error(str(tf)) / data_end['Beach Slope']

    # Incorporate 95% confidence interval from the regression into the error analysis. There are more
    # confidence intervals than profiles, remove the unnecessary ones
    ci_error_1 = ci_df[str(t0) + ' CI'].iloc[:len(x_error_yr1)]
    ci_error_2 = ci_df[str(tf) + ' CI'].iloc[:len(x_error_yr1)]

    # Convert to arrays
    x_error_yr1, x_error_yr2 = np.asarray(x_error_yr1, dtype=np.float64), np.asarray(x_error_yr2, dtype=np.float64)
    y_error_yr1, y_error_yr2 = np.asarray(y_error_yr1, dtype=np.float64), np.asarray(y_error_yr2, dtype=np.float64)
    ci_error_1, ci_error_2 = np.asarray(ci_error_1, dtype=np.float64), np.asarray(ci_error_2, dtype=np.float64)

    # Find the positional uncertainty based on the uncertainty in X and Y
    up1 = np.sqrt((x_error_yr1 ** 2) + (y_error_yr1 ** 2) + (ci_error_1 ** 2))
    up2 = np.sqrt((x_error_yr2 ** 2) + (y_error_yr2 ** 2) + (ci_error_2 ** 2))
...
def make_stats(data, avg, fences, param):
    ...
    # Create a .txt file
    partial_fname = f'(Figure 8) {param} Change Statistics.txt'
...
def main():
    ...
    # Load the data
    data_1997 = load_data(year=1997, variable='Beach Width')
    data_2010 = load_data(year=2010, variable='Beach Width')
    data_2016 = load_data(year=2016, variable='Beach Width')

    # Make a dataframe of Dhighs
    widths = change_df(d1997=data_1997, d2010=data_2010, d2016=data_2016)
    widths_errors = beach_width_change_df(widths=widths, rate=False)

    # Load a dataframe with fence locations identified
    fences = load_fence_locations(y=-7.5)

    # Make the figure
    y_min = -75
    y_text = y_min - (y_min * 0.35) + 3
    yticks = [y_min, -50, -25, 0, 25, 50, -y_min]
    make_overall_change_plot_w_kde(data=widths,
                                   fences=fences,
                                   param='Beach Width',
                                   y_min=y_min,
                                   y_max=y_min * -1,
    ...
    make_stats(widths, widths_errors, fences, 'Beach Width')
